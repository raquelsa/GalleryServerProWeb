            ...
            if (u == null)
            {
                ...
                using (SqlCeConnection conn = new SqlCeConnection(connectionString))
                {
                           ...
                           ,@Comment)", conn))
                    {
                        ...
                        cmd.Parameters.Add("@Email", SqlDbType.NVarChar, 256).Value = email == null ? DBNull.Value : (object)email;
                        cmd.Parameters.Add("@LoweredEmail", SqlDbType.NVarChar, 256).Value = email == null ? DBNull.Value :  (object)email.ToLowerInvariant();
        ...
        public override void UpdateUser(MembershipUser user)
        {
            ...
            using (SqlCeConnection conn = new SqlCeConnection(connectionString))
            {
                using (SqlCeCommand cmd = new SqlCeCommand("UPDATE [aspnet_Membership] " +
                        ...
                        " WHERE UserId = @UserId AND ApplicationId = @ApplicationId", conn))
                {
                    cmd.Parameters.Add("@Email", SqlDbType.NVarChar, 256).Value = user.Email == null ? DBNull.Value : (object)user.Email;
                    cmd.Parameters.Add("@LoweredEmail", SqlDbType.NVarChar, 256).Value = user.Email == null ? DBNull.Value : (object)user.Email.ToLowerInvariant();
...
//                        newCmd.Parameters.Add("@Email", SqlDbType.NVarChar, 256).Value = email == null ? DBNull.Value : (object)email;
//                        newCmd.Parameters.Add("@LoweredEmail", SqlDbType.NVarChar, 256).Value = email == null ? DBNull.Value : (object)email.ToLowerInvariant();
