...
namespace GalleryServerPro.Business
{
	...
	public class GallerySettings : IGallerySettings
	{
		...
		private int _maxAlbumThumbnailTitleDisplayLength;
		private int _maxMediaObjectThumbnailTitleDisplayLength;
		private string _mediaObjectCaptionTemplate;
		...
		private string[] _defaultRolesForSelfRegisteredUser;
		...
		{
			get { return _maxAlbumThumbnailTitleDisplayLength; }
			set { _maxAlbumThumbnailTitleDisplayLength = value; }
		}
		...
		{
			get { return _maxMediaObjectThumbnailTitleDisplayLength; }
			set { _maxMediaObjectThumbnailTitleDisplayLength = value; }
		}
		...
		public string MediaObjectCaptionTemplate
		{
			get { return _mediaObjectCaptionTemplate; }
			set { _mediaObjectCaptionTemplate = value; }
		}
		...
		{
			get { return _silverlightFileTypes; }
			set { _silverlightFileTypes = ToLowerInvariant(value); }
		}
		...
		/// the <see cref="IGallerySettings.ThumbnailClickShowsOriginal"/> property. That is, if this property is <c>false</c> and
		/// <see cref="IGallerySettings.ThumbnailClickShowsOriginal"/> is <c>true</c>, the user is shown the compressed version rather than 
		/// the original.
		/// </summary>
		public bool AllowAnonymousHiResViewing
		{
			get { return _allowAnonymousHiResViewing; }
			set { _allowAnonymousHiResViewing = value; }
		}
		...
		public string GpsMapUrlTemplate
		{
			get { return _gpsMapUrlTemplate; }
			set { _gpsMapUrlTemplate = value; }
		}
		...
		{
			get { return _enablePermalink; }
			set { _enablePermalink = value; }
		}
		...
		/// If <see cref="IGallerySettings.AllowAnonymousHiResViewing"/> is <c>false</c>, the original image is never shown to 
		/// anonymous users, even if this property is <c>true</c>.
		/// </summary>
		public bool ThumbnailClickShowsOriginal
		{
			get { return _thumbnailClickShowsOriginal; }
			set { _thumbnailClickShowsOriginal = value; }
		}
		...
		{
			get { return _thumbnailWidthBuffer; }
			set
			{
				if ((value < 0) || (value > 1000))
				{
					throw new ArgumentOutOfRangeException("value", String.Format(CultureInfo.CurrentCulture, "Invalid ThumbnailWidthBuffer setting: The value must be between 0 and 1,000. Instead, the value was {0}.", value));
				}

				_thumbnailWidthBuffer = value;
			}
		}
		...
		{
			get { return _thumbnailHeightBuffer; }
			set
			{
				if ((value < 0) || (value > 1000))
				{
					throw new ArgumentOutOfRangeException("value", String.Format(CultureInfo.CurrentCulture, "Invalid ThumbnailHeightBuffer setting: The value must be between 0 and 1,000. Instead, the value was {0}.", value));
				}

				_thumbnailHeightBuffer = value;
			}
		}
		...
		/// enumeration <see cref="GalleryServerPro.Business.PagerPosition" />, and must be one of the following values:
		...
		{
			get { return _defaultRolesForSelfRegisteredUser; }
			set { _defaultRolesForSelfRegisteredUser = value; }
		}
		...
		public void Initialize()
		{
			...
			if (!IsTemplate)
			{
				// Setting the FullMediaObjectPath property will throw an exception if the directory does not exist or is not writeable.
		...
		internal static IGallerySettingsCollection RetrieveGallerySettingsFromDataStore()
		{
			...
			foreach (GallerySettingDto gallerySettingDto in Factory.GetDataProvider().GallerySetting_GetGallerySettings())
			{
				...
				int currGalleryId = gallerySettingDto.FKGalleryId;

				if (!prevGalleryId.HasValue || (currGalleryId != prevGalleryId))
				{
					...
					bool isTemplate = gallerySettingDto.IsTemplate;

					gs = new GallerySettings(currGalleryId, isTemplate);
				...
				{
					AssignMetadataDisplaySettingsProperty(gs, prop, gallerySettingDto.SettingValue.Split(new char[] { ',' }, StringSplitOptions.RemoveEmptyEntries));
				}
		...
		{
			return Array.ConvertAll<string, string>(array, delegate(string s) { return s.ToLowerInvariant(); });
		}
	...
	public class GallerySettingsEventArgs : EventArgs
	{
		...
		public GallerySettingsEventArgs(int galleryId)
		{
			_galleryId = galleryId;
		}
