...
namespace GalleryServerPro.Web.Handler
{
	...
	public class getmedia : RangeRequestHandlerBase, IReadOnlySessionState
	{
			...
			get
			{
				switch (ResourceType)
				{
					...
					case MediaType.EmptyAlbumThumbnail:
						return Business.MimeType.LoadMimeType("dummy.jpg");
					default:
		...
		{
			get { return Path.GetFileName(MediaObjectFilePath); }
		}
		...
		private MediaType DetermineResourceType()
		{
			if (_mediaObjectId == int.MinValue)
			{
				// User specified moid=-2147483648 in the query string, which is the signal to request the empty album thumbnail.
		...
		private Bitmap GetDefaultThumbnailBitmap()
		{
			...
			try
			{
				...
				int x = (rctWidth - textSize.Width) / 2; //Starting point from left for the text
				int y = (rctHeight - textSize.Height) / 2; //Start point from top for the text
		...
		private Stream GetWatermarkedImageStream()
		{
			...
			try
			{
				...
				catch (Exception ex)
				{
					...
					watermarkedImage = Image.FromFile(_context.Request.MapPath(String.Concat(Utils.GalleryRoot, "/images/error_48x48.png")));
				}
