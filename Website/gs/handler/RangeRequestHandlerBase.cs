...
/// </summary>
/// <remarks>
/// Gallery Server Pro uses a heavily modified version of the handler created by:
/// 
		...
		using (Stream fs = GetResourceStream())
		{
			for (int i = 0; i < StartRangeBytes.Length; i++)
			{
				...
				while (bytesToReadRemaining > 0)
				{
					...
					{
						int chunkSize = fs.Read(buffer, 0, BufferSize < bytesToReadRemaining ? BufferSize : bytesToReadRemaining);
